...
geos = gp.GeoDataFrame.from_file('C:\Users\Amanda\NYC_Raw\Blocks_Clipped.shp')
population = 'C:\NYU\GRA\Data\Raw\Population_Data\Blocks\DEC_10_SF1_P1_with_ann.csv'
Output = 'C:\NYU\GRA\Data\Output_Shapefiles\Blocks_Pop_Fraction'
...
pop_pop = np.array(pop_pd['Total'])
pop_geo = np.zeros(len(zips_geo))

pop_pop_foo = np.array([i if 'r' not in i else i.split('r')[0][:-1] for i in pop_pop])


for ii,zz in enumerate(zips_geo):
	#print ii
	pop_geo[ii] = pop_pop_foo[zips_pop.index(zz)]
