...
# dictionary with zip code as key, total establishments as value
est = {}
...
for entry in emp:
	...
	for i in range(0, len(emp[entry])):
		...
		establishments = est[entry][i]
		try:
			empPerEst = totalEmployees/establishments
		except:
			empPerEst = 0.0
		epe[entry].append(empPerEst)

nYears = len(epe['10001'])

# Create ordered dictionary of salary
oepe = collections.OrderedDict(sorted(epe.items()))

# Create numpy array of feature vectors that is consistent with the
# ordered list of zip codes.
i = 0
fv = np.zeros(shape = (len(oepe), nYears))
for zipCode in oepe:
	for j in range(0, nYears):
		fv[i][j] = epe[zipCode][j]
	i += 1
...
text(-14.5, 12.5, 'Employees per\nEstablishment\n k = 5', size=16)

# save plot to pdf:
from matplotlib.backends.backend_pdf import PdfPages
pp = PdfPages('empPerEst.pdf')
...
gdf.to_file('../output/clusterPlots/shapefiles/employeesPerEst.shp')
...
title('Employees per Establishment, k = 2', size = 16)
ax1 = subplot(211)
...
gdf2.to_file('../output/clusterPlots/shapefiles/empPerEstK2.shp')
